...
@app.route("/api/admin/test-ceara-imap", methods=["GET"])
@admin_required
def api_admin_test_ceara_imap():
    """Testa conexao IMAP da Caixa 2 do ambiente do Railway com diagnostico completo."""
    ...
    port2 = int(os.environ.get("CEARA_IMAP_PORT_2", 993))
    result = {"server": s2, "port": port2, "user": u2, "pass_len": len(p2)}
    # 1) DNS
    try:
        ip = socket.gethostbyname(s2)
    ...
    try:
        ...
        sock = socket.create_connection((s2, port2), timeout=10)
        sock.close()
        result["tcp"] = {"ok": True}
    except Exception as e:
    ...
    try:
        ...
        sock = socket.create_connection((s2, port2), timeout=15)
        ssock = ctx.wrap_socket(sock, server_hostname=s2)
        result["ssl"] = {"ok": True, "version": ssock.version()}
        ssock.close()
    except Exception as e:
        result["ssl"] = {"ok": False, "error": str(e)}
    # 4) IMAP login
    try:
        m = imaplib.IMAP4_SSL(s2, port2)
        # capabilities ANTES do login
        try:
            result["capabilities"] = str(m.capabilities)[:300]
        except: pass
        try:
            r = m.login(u2, p2)
            result["login"] = {"ok": True, "response": str(r)}
            m.select("INBOX", readonly=True)
            typ, data = m.search(None, "ALL")
            if typ == "OK":
                result["inbox_count"] = len(data[0].split()) if data and data[0] else 0
            m.logout()
        except imaplib.IMAP4.error as e:
            result["login"] = {"ok": False, "imap_error": str(e)}
            try: m.logout()
            except: pass
        except Exception as e:
            result["login"] = {"ok": False, "error": str(e)}
    except Exception as e:
        result["login"] = {"ok": False, "connect_error": str(e)}
    return jsonify(result)
